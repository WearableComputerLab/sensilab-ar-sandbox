...
Shader "Unlit/SandboxShaderFire"
{
	Properties
	{
	...
	SubShader
	{
		...
		Pass
		{
			...
			sampler2D _FireSurfaceTex;
			float4 _FireSurfaceTex_ST;
			...
			fixed4 frag(v2f i) : SV_Target
			{
				...
				fixed4 fireColor = tex2D(_FireSurfaceTex, i.uv_FireSurfaceTex);
				float greyscale = GetGrayscaleValue(fireColor.rgb);

				int useContrastColour = greyscale < 0.5 && _DynamicLabelColouring == 1;
				...
				fixed4 textColor = (1 - contourMapFrag.textIntensity) * fireColor +
					contourMapFrag.textIntensity * contrastColour;

				fixed4 finalColor = drawMajorContourLine == 1 ? contrastColour : fireColor;
